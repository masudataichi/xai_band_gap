...
df_train = pd.read_csv("df_exp_merged.csv")
mp_train = pd.read_csv("df_mp_merged.csv")


y_train = df_train.iloc[:,-1]
y_pred_train = mp_train.iloc[:,-1]
...
print("MAE")
print(np.mean(abs(pd.DataFrame(y_pred_train).iloc[:,0]-pd.DataFrame(y_train).iloc[:,0])))
print("R2")
print(r2_score(y_train, y_pred_train))
print("RMSE")
print(np.sqrt(mean_squared_error(y_train, y_pred_train)))
print("MAPE")
print(np.mean((np.abs(np.array(y_train)-np.array(y_pred_train))/np.array(y_train)*100)))


# ================================================================== #

err_list = (pd.DataFrame(y_pred_train).iloc[:,0]-pd.DataFrame(y_train).iloc[:,0]).to_list()
ae_list = abs(pd.DataFrame(y_pred_train).iloc[:,0]-pd.DataFrame(y_train).iloc[:,0]).to_list()
y_exp_list = pd.DataFrame(y_train).iloc[:,0].to_list()
y_pred_list = pd.DataFrame(y_pred_train).iloc[:,0].to_list()
...
exp_num_split_df.to_csv("exp_num_split_df.csv")
