...
# Best scores found:  -0.5797288303413641
...
for train_index, test_index in kf_cv.split(x_train):
    ...
    train_split.to_csv(f'mp_cv_dataset/train/split_train_{split_num}.csv', index=False)
    
    # テストセットの保存
    test_split = pd.concat([X_test_split, y_test_split], axis=1)
    test_split.to_csv(f'mp_cv_dataset/test/split_test_{split_num}.csv', index=False)
