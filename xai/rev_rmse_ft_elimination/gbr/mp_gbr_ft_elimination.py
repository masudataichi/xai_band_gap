...
# model = GradientBoostingRegressor(random_state=42, n_estimators=5000, learning_rate=0.07, max_depth=5)
model = GradientBoostingRegressor(random_state=42, n_estimators=3000, learning_rate=0.07, max_depth=5)


# 並び順に特徴量を格納するCSVファイルの読み込み
sorted_features_path = '../../rmse_pfi/gbr/csv/mp_sorted_best_selected_gbr_all_rev.csv'
...
plt.title("GBR (MP dataset)", fontsize=28)
plt.legend(fontsize=18)
plt.tight_layout()
plt.savefig("img/mp_gbr_ft_elimination.png")

# 結果をCSVファイルに保存
results_sorted_df.to_csv('csv/mp_sorted_gbr_ft_elimination.csv', index=False)
results_random_df.to_csv('csv/mp_random_gbr_ft_elimination.csv', index=False)
