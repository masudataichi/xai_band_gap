...
model = RandomForestRegressor(random_state=42, max_depth=20, max_features='sqrt', n_estimators=600)




# 並び順に特徴量を格納するCSVファイルの読み込み
sorted_features_path = '../../rmse_pfi/rfr/csv/exp_sorted_best_selected_rfr_all_rev.csv'
...
plt.tight_layout()
plt.savefig("img/exp_rfr_ft_elimination.png")

# 結果をCSVファイルに保存
results_sorted_df.to_csv('csv/exp_sorted_rfr_ft_elimination.csv', index=False)
results_random_df.to_csv('csv/exp_random_rfr_ft_elimination.csv', index=False)
