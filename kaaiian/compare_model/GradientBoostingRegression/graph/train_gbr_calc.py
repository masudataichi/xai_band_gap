...
y_pred_train = pd.read_csv("../predictions/gbr_train_calc_saved.csv", header=None)
y_pred = pd.read_csv("../predictions/gbr_test.csv", header=None)
...
# ae_df_sorted.to_csv("ae_df_sorted_calc.csv")
...
plt.title("GBR", fontsize=30)
plt.plot(x_line, y_line, linestyle="dashed", dashes=[7,8])
plt.plot(x_line, y_line_reg, color = "#1f77b4")
plt.xlim(-1, 13)
plt.ylim(-1, 13)
plt.tight_layout()
plt.savefig("train_gbr_calc_saved_widen.png")