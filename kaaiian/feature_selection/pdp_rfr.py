...
selected_feature = pd.read_csv("sorted_best_selected_rfr.csv").iloc[:,1].tolist()
...
for i in range(3):        
    my_plots = partial_dependence(model,       # column numbers of plots we want to show
                                    ...
                                    features=[selected_feature[i]], grid_resolution = 10)
    plt.figure(figsize=(6, 4))

    for j in range(len(my_plots['values'][0])-1):
        print(my_plots['values'][0][j] - my_plots['values'][0][j+1])
    plt.scatter(my_plots['values'][0], my_plots['average'][0])
    plt.title('Partial Dependence Plot (RFR)')
    plt.xlabel(selected_feature[i])
    plt.ylabel('Partial Dependence')
    plt.ylim(1.7, 2.7)
    plt.grid(alpha=.3)
    plt.savefig("pdp_rfr_" + str(i) + ".png")